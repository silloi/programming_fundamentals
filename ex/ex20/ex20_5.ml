...
let rec insert1 ekikan_tree kiten shuten kyori =
  let lst = try RedBlack.search ekikan_tree kiten with Not_found -> [] in
  RedBlack.insert ekikan_tree kiten ((shuten, kyori) :: lst)
...
let rec get_ekikan_kyori ekimei1 ekimei2 ekikan_tree =
  try
    let lst = RedBlack.search ekikan_tree ekimei1 in
